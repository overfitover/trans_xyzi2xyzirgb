...
def get_lidar_in_image_fov(pc_velo, calib, xmin, ymin, xmax, ymax,
                           return_more=True, clip_distance=2.0):
    ...
    if return_more:
        return imgfov_pc_velo, pts_2d, fov_inds  #  return fov velodyne, pc_image,fov indexs
    else:
        return imgfov_pc_velo

def show_lidar_with_boxes(pc_velo, calib,
                          img_fov=True, img_width=None, img_height=None):
    ''' return point cloud of image fov and fov indexs '''
    ...
    if img_fov:
        pc_velo,_,fov_inds = get_lidar_in_image_fov(pc_velo, calib, 0, 0,
...
def show_lidar_on_image(pc_velo, img, calib, img_width, img_height):
    ...
    for i in range(imgfov_pts_2d.shape[0]):
        ...
        rgb_list.append(img[int(np.round(imgfov_pts_2d[i, 1])) - 1][int(np.round(imgfov_pts_2d[i, 0])) - 1])
        # cv2.circle(img, (int(np.round(imgfov_pts_2d[i,0])),
        #     int(np.round(imgfov_pts_2d[i,1]))),
        #     2, color=tuple(color), thickness=-1)
        # print(int(np.round(imgfov_pts_2d[i,0])),int(np.round(imgfov_pts_2d[i,1])))
    # Image.fromarray(img).show()
    return img,rgb_list
