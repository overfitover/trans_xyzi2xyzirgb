...
if __name__=='__main__':
    # pc = np.loadtxt('000001.txt')
    # print("txt文件shape: ", pc.shape)
    # pc.tofile('02.bin')
    velo_bin = np.fromfile('000001.bin', np.float32)
    print('bin 文件shape: ', velo_bin.shape)
    point_cloud = velo_bin.reshape(-1, 7)
    print("reshape: ", point_cloud.shape)
    fig = draw_lidar(point_cloud)

    # pc = np.loadtxt('000001.txt')
    # print("txt shape", pc.shape)

    # velo = np.fromfile('000001.bin', dtype=np.float32)
    # print("bin shape:", velo.reshape(-1, 7).shape)
